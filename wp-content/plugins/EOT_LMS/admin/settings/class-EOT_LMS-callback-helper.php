...
class EOT_LMS_Callback_Helper {
	...
	private function get_id_attribute( $id ) {
		return ' id="' . $this->snake_cased_EOT_LMS . '_settings[' . $id . ']" ';
	}

	private function get_name_attribute( $name ) {
		return ' name="' . $this->snake_cased_EOT_LMS . '_settings[' . $name . ']" ';
	}
	...
	private function get_label_for( $id, $desc ) {
		return '<label for="' . $this->snake_cased_EOT_LMS . '_settings[' . $id . ']"> '  . $desc . '</label>';
	}
	...
	public function missing_callback( $args ) {
		printf( __( 'The callback function used for <strong>%s</strong> setting is missing. Please ensure the field \'type\' is correct', $this->EOT_LMS ), $args['id'] );
	}
	...
	public function checkbox_callback( $args ) {
		...
		$value = EOT_LMS_Option::get_option( $args['id'] );
	...
	public function multicheck_callback( $args ) {
		...
		if ( empty( $args['options'] ) ) {
			printf( __( 'Options for <strong>%s</strong> multicheck is missing.', $this->EOT_LMS ), $args['id'] );
			return;
		}

		$old_values = EOT_LMS_Option::get_option( $args['id'], array() );
	...
	public function radio_callback( $args ) {
		...
		if ( empty( $args['options'] ) ) {
			printf( __( 'Options for <strong>%s</strong> radio is missing.', $this->EOT_LMS ), $args['id'] );
			return;
		}

		$old_value = EOT_LMS_Option::get_option( $args['id'] );
	...
	private function input_type_callback( $type, $args ) {
		...
		$value = EOT_LMS_Option::get_option( $args['id'], $args['std']  );
	...
	public function rich_editor_callback( $args ) {
		...
		if ( $wp_version >= 3.3 && function_exists( 'wp_editor' ) ) {
			ob_start();
			wp_editor( stripslashes( $value ), $this->snake_cased_EOT_LMS . '_settings_' . $args['id'], array( 'textarea_name' => $this->snake_cased_EOT_LMS . '_settings[' . $args['id'] . ']' ) );
			$html = ob_get_clean();
		} else {
	...
	public function upload_callback( $args ) {
		...
		$html .= $this->get_id_and_name_attrubutes( $args['id'] );
		$html .= 'class="' . $args['size'] . '-text ' . $this->snake_cased_EOT_LMS . '_upload_field" ';
		$html .= ' value="' . esc_attr( stripslashes( $value ) ) . '"/>';

		$html .= '<span>&nbsp;<input type="button" class="' . $this->snake_cased_EOT_LMS . '_settings_upload_button button-secondary" value="' . __( 'Upload File', $this->EOT_LMS ) . '"/></span>';
