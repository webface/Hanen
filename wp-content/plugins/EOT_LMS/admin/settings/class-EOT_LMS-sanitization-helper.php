...
/**
 * EOT_LMS Sanitization Helper Class
 *
 ...
 */
class EOT_LMS_Sanitization_Helper {
	...
	public function __construct( $EOT_LMS ) {
		...
		add_filter( $this->snake_cased_EOT_LMS . '_settings_sanitize_text', array( $this, 'sanitize_text_field' ) );
		add_filter( $this->snake_cased_EOT_LMS . '_settings_sanitize_email', array( $this, 'sanitize_email_field' ) );
		add_filter( $this->snake_cased_EOT_LMS . '_settings_sanitize_checkbox', array( $this, 'sanitize_checkbox_field' ) );
		add_filter( $this->snake_cased_EOT_LMS . '_settings_sanitize_url', array( $this, 'sanitize_url_field' ) );
	}
	...
	/**
	 * Return the snake cased version of the EOT LMS
	 *
	 * @return string
	 */
	public function get_snake_cased_EOT_LMS() {
		return $this->snake_cased_EOT_LMS;
	}
	 ...
	 *
	 * Filters in order:
	 * - <snake_cased_EOT_LMS>_settings_sanitize_<tab_slug>
	 * - <snake_cased_EOT_LMS>_settings_sanitize_<type>
	 * - <snake_cased_EOT_LMS>_settings_sanitize
	 * - <snake_cased_EOT_LMS>_settings_on_change_<tab_slug>
	 * - <snake_cased_EOT_LMS>_settings_on_change_<field_key>
	 *
	 * @since 	1.0.0
	...
	public function settings_sanitize( $input = array() ) {
		...
		$tab = isset( $referrer['tab'] ) ? $referrer['tab'] : EOT_LMS_Settings_Definition::get_default_tab_slug();

		// Tab filter
		$input = apply_filters( $this->snake_cased_EOT_LMS . '_settings_sanitize_' . $tab, $input );
		...
		add_settings_error( $this->EOT_LMS . '-notices', $EOT_LMS, __( 'Settings updated.', $this->EOT_LMS ), 'updated' );
	...
	private function apply_type_filter( $input, $tab, $key ) {
		...
		return apply_filters( $this->snake_cased_EOT_LMS . '_settings_sanitize_' . $type, $input[$key], $key );
	}

	private function apply_general_filter( $input, $key ) {

		return apply_filters( $this->snake_cased_EOT_LMS . '_settings_sanitize', $input[$key], $key );
	}

	// Key specific on change hook
	private function do_settings_on_key_change_hook( $key, $new_value ) {

		$old_plugin_settings = get_option( $this->snake_cased_EOT_LMS . '_settings' );

		if ( $old_plugin_settings[$key] !== $new_value ) {

			do_action( $this->snake_cased_EOT_LMS . '_settings_on_change_' . $key, $new_value, $old_plugin_settings[$key] );
	...
	private function do_settings_on_change_hook( $new_values, $tab ) {
		...
		if ( $changed ) {
			...
			do_action( $this->snake_cased_EOT_LMS . '_settings_on_change_' . $tab, $new_values, $old_plugin_settings );
