            ...
            if (isset($_POST['submit'])) 
            {
                ...
                {//parse and update the resource order
                    if (strpos($k, 'order_') === 0) 
                    {
                        ...
                        $id = $parts[1];
                        $upd = $wpdb->update(TABLE_MODULE_RESOURCES, array('order' => $v), array('resource_id' => $id));
                    }
//                    if ($v > $max)//attempt to get the next max order number 
//                    {
//                        $max = $v;
//                    }
                }
            ...
            $resources = getUserUploads($org_id, $user_id);
            //$quizzes = $eot_quiz->getQuizzes($org_id, $user_id);
            $quizzes= array();
            $module_resources = getResourcesInModule($module_id);
            // var_dump($module_resources);
            ?>
                    ...
                    <table class="table table-hover table-bordered table-striped files">
                        ...
                        <tbody>
                            ...
                            foreach ($module_resources as $module_resource) 
                            {
                                ...
                                echo'<td><a href="' . $admin_ajax_url . '?action=get_module_form&form_name=delete_resource&resource_id=' . $module_resource['resource_id'] . '&subscription_id=' . $subscription_id . '" class="delete" rel="facebox"><span class="fa fa-minus-circle"></span></a></td>';
                                echo "</tr>";
                            }
                            ?>
                        </tbody>
                ...
                <div class="bs">
                    ...
                    <div class="btns" style="display:none">
                        <label for="order">Order
                            <input id="order" name="order" value="<?=  1 ?>" class="form-control form-group-sm" type="text" onkeypress='return event.charCode >= 48 && event.charCode <= 57'/>(higher numbers at the bottom)
                        </label>
                ...
                <script>
                    ...
                    function rename_module() {
                        ...
                        $.facebox(function () {
                            ...
                            $.ajax({
                                data: {'title': $('#moduleTitle').html(), 'module_id':<?= $module_id; ?>},
                                error: function () {
