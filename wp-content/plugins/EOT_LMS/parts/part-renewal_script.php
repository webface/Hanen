...
$query = "
        	SELECT u.ID as ID, u.display_name,u.user_email,uma.meta_value AS 'first_name', umb.meta_value AS 'last_name', 'manager' as role, s.trans_date, s.dash_price, s.id AS 'subscription_id'  
        	FROM wp_users u 
...
foreach ($managers as $manager) {
    ...
    $usersTableObj->rows[] = array(
        ...
        $manager['trans_date'],
        $manager['dash_price'],
        wp_hash($manager['user_email'].$manager['subscription_id'])
    );
   // d($manager['user_email'].$manager['subscription_id']);
}
CreateDataTable($usersTableObj, "100%", 10, true, "Renewal List"); // Print the table in the page
