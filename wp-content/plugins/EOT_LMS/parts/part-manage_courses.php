...
?>
      ...
      <div style="float:left;" id="group_list" class="holder osX" org_id="<?= $org_id ?>" subscription_id="<?= $subscription_id ?>"> 
          ...
          <div id="pane2" class="scroll-pane" style="overflow: hidden; height: 250px; padding-right: 0px;">
            <div style="width:100%;">
              <?php 
                ...
                ?>
                      <div class="group_list_table_row" group_id="<?= $course_id ?>" course-id="<?= $course_id ?>" org-id="<?= $org_id ?>" subscription-id="<?= $subscription_id?>">
...
//          })  
          ...
          *******************************************************************************************/    
          ...
          $(document).on('click','#create_staff_fb',function(){
            ...
            var url =  ajax_object.ajax_url + "?action=getCourseForm&form_name=create_staff_account&org_id="+org_id+"&group_id="+group_id+"&group_name="+group_name+'&subscription_id='+sub_id;
          ...
          ****************************************************************/
          ...
          $(document).on('click','#show_add_to_group',function(){
            if($("#staff_and_assignment_list").attr("group_id")!="null")
            {
              ...
              enrollment_id = 0;
              var url =  ajax_object.ajax_url + "?action=getCourseForm&form_name=add_staff_to_group&org_id="+org_id+"&subscription_id="+subscription_id+"&group_id="+group_id+"&group_name="+encodeURIComponent(group_name);
              jQuery.facebox(
              function(){
          ...
          *******************************************************************************************/
          ...
          $(document).on('click','#show_edit_videos_in_group',function(){
            if($("#staff_and_assignment_list").attr("group_id")!="null")
            {
              ...
              sub_id = $("#group_list").attr("subscription_id");
              group_name = group_name_global;
              var ajax_url = ajax_object.ajax_url + "?action=getCourseForm&form_name=add_video_group&org_id="+org_id+"&course_id="+group_id+"&course_name="+encodeURIComponent(group_name)+"&subscription_id="+sub_id;  
              jQuery.facebox(
                function()
                {
                  $.ajax({
                  ...
                  $('#video_listing_pane').css({'height':'550px'}).jScrollPane()
                      ...
                      .click(
                        ...
                        $.getJSON(''+ajax_object.ajax_url+'?action=toggleItemInAssignment&group_id='+obj.attr("group_id")+'&item='+obj.attr("item")+'&item_id='+obj.attr("item_id")+'&module_id='+obj.attr("video_id")+'&org_id='+obj.attr("org_id"),
                          function(json)
                          {
                          if(json.action=="added")
                          {
...
//                      ) 
          ...
          *****************************************************/
          $(document).bind('success.create_staff_group',
            function(event,data)
            {
              ...
              var found = false;
              var i=0;
              var div = '<div class = \"group_list_table_row\" group_id = '+data.group_id+' course-id= '+data.group_id+' org-id= '+data.org_id+' subscription-id='+data.subscription_id+' course-status=\"draft\"> \
        ...
        *****************************************************/
        function redirect_to_main() {
          ...
          var url =  ajax_object.ajax_url + "?action=getCourseForm&form_name=add_staff_to_group&org_id="+org_id+"&group_id="+group_id+"&group_name="+encodeURIComponent(group_name);
