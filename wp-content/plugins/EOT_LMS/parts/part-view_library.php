<?php
  if(isset($_REQUEST['subscription_id']) && $_REQUEST['subscription_id'] > 0)
  {
      ...
      $result = getSubscriptions($subscription_id,0,1);
      $master_modules = getModulesByLibrary(1); // Get all the modules from the master library (course).
    if($result)
    {
      $subscription = $result; // get the current subscription
      $result = getLibrary ($subscription->library_id); // The library information base on the user current subscription
      if($result)
      {
        $library = $result; // The library information base on the user current subscription
        ...
        <span>
          ...
          <a href="https://www.expertonlinetraining.com/choosing-a-topic/">Click here</a> to see a listing of which topics are included in these pre-made courses.
          <br><br>
          ...
          the videos, quizzes, and handouts created by our amazing faculty. To view a table of recommended videos for different groups of staff, <a href="https://www.expertonlinetraining.com/choosing-a-topic/">click here</a>.
          <br><br>
