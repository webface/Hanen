<?php
		if(isset($_REQUEST['course_id']) && $_REQUEST['course_id'] != "")
		{
		    ...
		    {
		    	// Assuming to get the course info from part-default via POST
	    		if( isset($_REQUEST['course']) )
				{
					$course =  json_decode( stripslashes( $_REQUEST['course'] ) ); // The course information
				}
				else
				{
				// Assuming redirect coming from part-view.php by breadcrumbs. Need to generate getCourse again.
					$course = json_decode ( json_encode( getCourse($course_id) ), FALSE); // Convert course into object.
				}
				if($course)
				{
		       	?>
					...
					<h2>Course "<u><?= $course->course_name ?></u>"</h2> 	
