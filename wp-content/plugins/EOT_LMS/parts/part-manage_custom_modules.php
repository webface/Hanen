<div class="breadcrumb">
    ...
    <?= CRUMB_ADMINISTRATOR ?>    
    <?= CRUMB_SEPARATOR ?>    
    <span class="current">Manage Your Custom Content</span>     
</div>
<?php
...
{
    if (!isset($_POST['submit_module']) || !wp_verify_nonce($_POST['submit_module'], 'submit_module')) 
    {
        print 'Sorry, your nonce did not verify.';
        exit;
    }
    $data = array(
        'title' => filter_var($_POST['moduleName'], FILTER_SANITIZE_STRING),
        'description_text' => filter_var($_POST['moduleDescription'], FILTER_SANITIZE_STRING),
        'org_id' => filter_var($_POST['org_id'], FILTER_SANITIZE_NUMBER_INT),
        'creator_id' => filter_var($_POST['user_id'], FILTER_SANITIZE_NUMBER_INT),
...
if (isset($_REQUEST['subscription_id']) && $_REQUEST['subscription_id'] != "") 
{
    ...
    if (isset($true_subscription['status']) && $true_subscription['status']) 
    {
        ...
        {
            ?>
            <h1 class="article_page_title">Manage Your Custom Content</h1>
            <h3>Your Modules</h3>
            <?php
            if ($modules)// then display datatable 
            {
                ...
                foreach ($modules as $module) 
                {
                    $moduleTableObj->rows[] = array(
                        ...
                        '<a href="/dashboard?part=edit_module&module_id=' . $module['ID'] . '&subscription_id=' . $subscription_id . '" onclick="load(\'load_edit_quiz\')">Edit module</a> | 
                        <a href="' . $admin_ajax_url . '?action=get_module_form&form_name=delete_module&module_id=' . $module['ID'] . '&subscription_id=' . $subscription_id . '" class="delete" rel="facebox"><span class="fa fa-minus-circle"></span></a>'
                    );
                }
