<?php
  if(isset($_REQUEST['user_id']) && $_REQUEST['user_id'] != "" && current_user_can("is_sales_manager"))
  {
    ...
    $email = 'deactivate_' . $user->user_email;	//email
    $randompw = wp_generate_password(); //random password
...
?>
  	<div class="breadcrumb">
          ...
          <?= CRUMB_SEPARATOR ?>
      	<span class="current"Deactivate Sales Rep</span>     
  	</div>
    <img style="margin-right:60em;" class="loader" src="<?= get_template_directory_uri() . "/images/loading.gif"?>" hidden>
    <div class="main-content">
  	  <h1 class="article_page_title">Deactivate Sales Representative</h1>
      <form id="deactivate_sales_rep" method="POST" action="">
        <table class="data small">
          <tbody>
              <tr>
                ...
                <td>
                 <input type="text" name="first_name" size="35" value="<?= $first_name ?>" readonly>
                </td>
              </tr>
              <tr>
                <td class="label right">
                ...
                <td>
                 <input type="text" name="last_name" size="35" value="<?= $last_name ?>" readonly>
                </td>
              </tr> 
              <tr>
                <td class="label right">
                ...
                <td>
                  <input type="text" name="email" id="email" value="<?= $email ?>" size="35" readonly>
                </td>
              </tr>
              <tr>
                <td class="right" colspan=2>
              	<input type="submit" class="deactivate_sales" value="Deactivate User">
    ...
    <script>
      ...
      $("#deactivate_sales_rep").submit(function(e) {
	    ...
	    $.ajax({
	      ...
	      url: url,
	      dataType: 'json',
	      data: $("#deactivate_sales_rep").serialize(),
	      success:
	      function(data)
	      {
            if (!data['status'])
