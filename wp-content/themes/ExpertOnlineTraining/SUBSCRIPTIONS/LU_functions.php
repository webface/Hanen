...
// curl -X POST "https://api.cloudflare.com/client/v4/zones/090bf1cc1d85e6ff1d545c52c7b865b2/dns_records" -H "X-Auth-Email: hagai@targetdirectories.com" -H "X-Auth-Key: d0b867e584fe89824a58cfe3153d1fd1e6fe1" -H "Content-Type: application/json" --data '{"type":"CNAME","name":"udicamp","content":"eotudicamp.learnupon.com","ttl":1}'
    ...
    else
    {
        ...
        LU_sendMail('cloudflare_error', $recepients, $data);
...
//            return createPortal($org_name, $number_of_licenses, $logo_image_url, $org_subdomain, $data);
    ...
    case "activate_account": {
            ...
            return LU_activatePortalAccount($data);
            break;
    }
        case "clone_courses": {
            /********************************************************
 ...
 **/
function LU_activatePortalAccount($data = array())
{
  extract ($data);
  ...
  if (isset($response['message'])) {
    return array('status' => 0, 'message' => "LUERROR in LU_activatePortalAccount: " . $response['message']);
  } else if ($response['id']) {
//    return array('status' => 1); //dont return yet because we need to add # licenses to portal
  } else {
    return array('status' => 0, 'message' => "LUERROR in LU_activatePortalAccount: There was a problem. Please try again later.\n" . print_r ($response, true));
  }
  ...
  //checks for errors when creating the portal
  if(isset($response['message'])) {
      return array('status' => 0, 'message' => "LUERROR in LU_activatePortalAccount->update Licenses: " . $response['message']);
  }
  else if($response['id']) {
//    return array('status' => 1); // dont return yet because we need to clone the courses into the portal
  }
  else {
      return array('status' => 0, 'message' => "LUERROR in LU_activatePortalAccount->update Licenses: There was a problem. Please try again later.");
  }
        ...
        foreach ($base_courses as $course_name => $LU_course_ID)
        {
            ...
            if ($LU_course_ID && $org_lrn_upon_id)
            {
                ...
                if (isset($response['status']) && !$response['status']) {
                    return array('status' => 0, 'message' => 'LUERROR in LU_activatePortalAccount: Couldnt cloneCourse: $course_name -> $LU_course_ID ' . $response['message']);
                }
            }
        }
...
/* THIS IS MORE INVOLVED AS IT REQUIRES CHANGES TO CHECKING COMPLETED ENROLLMENTS AS WELL. FOR NOW LEAVE IT AS LE LIBRARY.
      else if ($subscription->library_id == LE_SP_DC && !$cloned)
      {
        ...
        if (isset($response['status'] && !$response['status']) {
            return array('status' => 0, 'message' => 'LUERROR in LU_activatePortalAccount: Couldnt cloneCourse: LE SP DC' . $response['message']);
        }
        // delete the Leadership Essentials Library
        $response = LU_delete_course( , $portal_subdomain);
      } 
*/      
    }
  }
 ...
 */
function LU_delete_course($course_id, $portal_subdomain = DEFAULT_SUBDOMAIN, $data = array()){
  extract($data);
  ...
  if(isset($response['message'])) 
  {
    return array('status' => 0, 'message' => "LUERROR in LU_delete_course: " . $response['message']);
  }
  else if($response['id']) 
  {
  ...
  {
    return array('status' => 0, 'message' => "LUERROR in LU_delete_course: There was a problem. Please try again later.");
  }
}
 ...
 */
function LU_getCourses($portal_subdomain = DEFAULT_SUBDOMAIN, $draft = 0, $data = array()) {
  extract($data);
  /*
...
//    $portal_password = get_post_meta ($org_id, 'lrn_upon_api_pass', true);
  ...
  if(isset($response['message'])) 
  {
    return array('status' => 0, 'message' => "LUERROR in LU_getCourses:" . $response['message']);
  }    
 ...
 */
function LU_getModules($course_id = 0, $portal_subdomain = DEFAULT_SUBDOMAIN, $data = array(), $component_type = '') {
  extract($data);
  ...
  if(isset($response['message'])) 
  {
    return array('status' => 0, 'message' => "LUERROR in LU_getModules:" . $response['message']);
  }    
 ...
 *  @return json encoded list of modules
 */
function LU_deleteModule($course_id = 0, $portal_subdomain = DEFAULT_SUBDOMAIN, $data = array()) {
  extract($data);
   ...
   */
  if($course_id <= 0)
    return array('status' => 0, 'message' => "Error in LU_deleteModule: Invalid course id");
  ...
  {
    return array('status' => 0, 'message' => "Error in LU_deleteModule: No portal subdomain");
  }

  //checks for errors when deleting the module
  if(isset($response['message'])) 
  {
    return array('status' => 0, 'message' => "LUERROR in LU_deleteModule:" . $response['message']);
  }    
 ...
 *  @return json encoded list of modules
 */
function LU_addModule($course_id = 0, $portal_subdomain = DEFAULT_SUBDOMAIN, $data = array()) {
  extract($data);
  ...
  {
    return array('status' => 0, 'message' => "Error in LU_AddModule: No portal subdomain");
  }

  //checks for errors when adding the module
  if(isset($response['message'])) 
  {
    return array('status' => 0, 'message' => "LUERROR in LU_AddModule: " . $response['message']);
  }    
  else if (isset($response['id'])) 
  {
 ...
 *  @return json encoded list of modules
 */
function LU_toggleItemInAssignment($course_id = 0, $portal_subdomain = DEFAULT_SUBDOMAIN, $data = array()) {
  extract($data);
  /*
   * Variables required in $data
   * org_id - the organization ID
  ...
  $modules = LU_getModules( $course_id, $portal_subdomain, $info_data ); // All the modules registered in the course.
  if (empty($modules))
  {
    $modules = array();
  }
  ...
  {
    return LU_deleteModule($course_id, $portal_subdomain, $info_data);
  }
  else
  {
    return LU_addModule($course_id, $portal_subdomain, $info_data);
  }
 ...
 *  @return json encoded list of modules
 */
function LU_getCourseByName($course_name, $portal_subdomain = DEFAULT_SUBDOMAIN, $data = array()) {
  extract($data);
  ...
  {
    return array('status' => 0, 'message' => "LUERROR in LU_getCourseByName: course name can't be empy or null.");
  }
  else 
  {
    $courses = LU_getCourses($portal_subdomain, 1, $data); // All the courses that are in in draft mode.
    ...
    {
        return array('status' => 0, 'message' => "LUERROR in LU_getCourseByName: couldn't find the course.");
    }
  }
}
 ...
 */
function LU_getUsersInCourse($course_id = 0, $portal_subdomain = DEFAULT_SUBDOMAIN, $data = array()){
    extract($data);
    ...
    */
    ...
    {
        return array('status' => 0, 'message' => "ERROR in LU_getUsersInCourse: Invalid course.");
    }
    else 
    {
    ...
    if(isset($response['message'])) 
    {
        return array('status' => 0, 'message' => "LUERROR in LU_getUsersInCourse:" . $response['message']);
    }    
 ...
 *  @return json encoded list of modules
 */
function LU_updateCourse($course_id = 0, $portal_subdomain = DEFAULT_SUBDOMAIN, $data) {
  extract($data);
  ...
  if ($course_id == 0) // check if course_id is set, otherwise list all modules in the portal
  {
    return array('status' => 0, 'message' => "ERROR in LU_UpdateCourse: No course_id submitted.");
  }
  ...
  if(isset($response['message'])) 
  {
    return array('status' => 0, 'message' => "LUERROR in LU_updateCourse:" . $response['message']);
  }    
  else if (isset($response['id'])) 
  {
 ...
 */
function LU_createCourse($course_name = '', $portal_subdomain = DEFAULT_SUBDOMAIN, $data) {
    extract($data);
    /*
     * Variables required in $data
     * org_id - the organization ID
    ...
    {
        return array('status' => 0, 'message' => "Error in LU_createCourse: No portal subdomain");
    }
    ...
    if(isset($response['message'])) 
    {
      return array('status' => 0, 'message' => "LUERROR in LU_createCourse:" . $response['message']);
    }    
    else if (isset($response['id'])) 
    {
      return array('status' => 1, 'id' => $response['id']);
 ...
 */
function LU_deleteCourse($course_id = 0, $portal_subdomain = DEFAULT_SUBDOMAIN, $data) {
  extract($data);
  ...
  {
    return array('status' => 0, 'message' => "ERROR in LU_deleteCourse: No course_id submitted.");
  }
  ...
  $enrollments = LU_getUsersInCourse($course_id, $portal_subdomain, $data);
  $unenroll_error = '';

  //unenroll users one by one before deleting the course
  foreach ($enrollments as $enrollment)
  {
    $response = LU_deleteEnrolledUser($enrollment['id'], $portal_subdomain, $data);

    if(isset($response['message'])) 
    {
//      return array('status' => 0, 'message' => "LUERROR in LU_deleteCourse while deleting enrollment:" . $response['message']);
  ...
  if(isset($response['message'])) 
  {
    return array('status' => 0, 'message' => "LUERROR in LU_deleteCourse:" . $response['message']);
  }    
  else 
  {
    return array('status' => 1);
 ...
 */
function LU_createUser($portal_subdomain = DEFAULT_SUBDOMAIN, $data) {
    extract($data);
    /**
    ...
    if(isset($response['message'])) 
    {
      return array('status' => 0, 'message' => "LUERROR in LU_createUser: " . $response['message']);
    }    
    else if (isset($response['id'])) 
    {
      return array('status' => 1, 'id' => $response['id']);
 ...
 */
function LU_massMail ( $sender_email = '', $sender_name = '', $recipients = array()) {
    ...
    if( $sender_email != null && $sender_email != "" && $sender_name != null && $sender_name != "")
    {
        ...
        foreach ($recipients as $recipient)
        {
            ...
            {
              $status = 0;
              error_log('LU_massmail error: to: ' . $recipient['email'] . ' num reciepients: ' . count($recipients));
            }
        }       
    }
    else
    {
        return array('status' => 0, 'message' => "LU_massMail error: invalid sender name/email.");
    }
    ...
    {
        return array('status' => 0, 'message' => 'LU_massMail error: Your messsage failed to send.');
    }
}
 ...
 */
function LU_sendMail ( $target = '', $recipients = array(), $data ) {
    extract($data);
    ...
    if( $target != null && $target != "" )
    {
        ...
        if (count( $recipients ) > 0)
        {
            ...
            {  
                ...
                return LU_massMail($sender_email, $sender_name, $recipients);
            }
            else if($target == "NewSubscription" || $target == "NewAccount")
            {
                return LU_massMail(get_bloginfo( 'admin_email' ),'Expert Online Training', $recipients);
            }
            else if($target == "cloudflare_error")
            {
                return LU_massMail(get_bloginfo( 'admin_email' ), 'Couldflare error', $recipients);
            }
            else if($target == "LU_massmail" )
            {
              return LU_massMail($sender_email, $sender_name, $recipients);
            }
        }
        ...
        {
            // no receipients
            return array('status' => 0, 'message' => "LU_sendMail error: no recipients.");
        }
    }
    else 
    {
        // No target defined. Return error.
        return array('status' => 0, 'message' => "LU_sendMail error: invalid target.");
    }
}
 ...
 **/
function LU_updateUser($portal_subdomain, $data){
    extract($data);
    /*
     * Variables required in $data
     * org_id - the organization ID
    ...
    if(isset($response['message'])) 
    {
      return array('status' => 0, 'message' => "LUERROR in LU_updateUser:" . $response['message']);
    }    
    else if (isset($response['id'])) 
    {
      return array('status' => 1, 'id' => $response['id']);
 ...
 */
function LU_getEnrollment($course_id = 0, $portal_subdomain = DEFAULT_SUBDOMAIN, $data = array()) {
  extract($data);
  ...
  {
    return array('status' => 0, 'message' => "ERROR in LU_getEnrollment: Invalid course ID");
  }
  ...
  if(isset($response['message'])) 
  {
    return array('status' => 0, 'message' => "LUERROR in LU_getEnrollment:" . $response['message']);
  }    
  else if (isset($response['enrollments'][0]['id'])) 
  {
    return $response['enrollments'];
 ...
 */
function LU_getEnrollmentStatusByCourseName($course_name = '', $portal_subdomain = DEFAULT_SUBDOMAIN, $data = array()) {
    extract($data);
    /*
    * Variables required in $data
    * org_id - the organization ID 
    */
    if($course_name == '')
    {
        return array( 'status' => 0, 'message' => "ERROR in LU_getEnrollmentStatusByCourseName: course name cannot be empty " );
    }
    else
    {
    ...
    if(isset($response['message'])) 
    {
        return array('status' => 0, 'message' => "LUERROR in LU_getEnrollmentStatusByCourseName: " . $response['message']);
    }  
 ...
 *******************************************************************************************************/
add_action('wp_ajax_LU_deleteStaffAccount', 'LU_deleteStaffAccount_callback');
function LU_deleteStaffAccount_callback () {
    if( isset ( $_REQUEST['org_id'] ) && isset ( $_REQUEST['portal_subdomain'] ) && isset ( $_REQUEST['staff_id'] ) && isset ( $_REQUEST['email'] ) )
    {
        // This form is generated in LU_LU_getCourseForm function with $form_name = change_course_status_form from this file.
        ...
        if( ! wp_verify_nonce( $_POST['_wpnonce'] ,  'delete-staff_id-org_id_' . $org_id ) ) 
        {
            ...
            $result['errors'] = 'LU_deleteStaffAccount_callback error: Sorry, your nonce did not verify.';
        }
        else if( !current_user_can ('is_director') && !current_user_can ('is_sales_rep') && !current_user_can('is_sales_manager') )
        {
            $result['display_errors'] = 'failed';
            $result['success'] = false;
            $result['errors'] = 'LU_deleteStaffAccount_callback Error: Sorry, you do not have permisison to view this page.';
        }
        else
        {
            ...
            if(isset($response['status']) && $response['status'])
            {
                ...
                if($user)
                {
                    ...
                    else
                    {
                        $result['display_errors'] = 'failed';
                        $result['success'] = false;
                        $result['errors'] = 'LU_deleteStaffAccount_callback ERROR: Could not delete the WP user account.';
                    }
                }
                else
                {   
                    $result['display_errors'] = 'failed';
                    $result['success'] = false;
                    $result['errors'] = 'LU_deleteStaffAccount_callback ERROR: Could not find the WP user account.';
                }
            }
            else
            {
              $result['display_errors'] = 'failed';
              $result['success'] = false;
              $result['errors'] = 'LU_deleteStaffAccount_callback ERROR: Could not delete the LU user account.';
            }
        }
    }
    else
    {
        $result['display_errors'] = 'failed';
        $result['success'] = false;
        $result['errors'] = 'LU_deleteStaffAccount_callback ERROR: Missing some parameters.';
    }
 ...
 *   Updating user info.
 */  
add_action('wp_ajax_LU_updateUser', 'LU_updateUser_callback'); //handles actions and triggered when the user is logged in
function LU_updateUser_callback() {
    if( isset ( $_REQUEST['name'] ) && isset ( $_REQUEST['lastname'] ) && isset($_REQUEST['email']) && isset ( $_REQUEST['portal_subdomain'] ) && isset ( $_REQUEST['old_email'] ))
    {
        ...
        if( ! wp_verify_nonce( $_REQUEST['_wpnonce'] ,  'update-staff_' . $user_id ) ) 
        {
            $result['display_errors'] = 'failed';
            $result['success'] = false;
            $result['errors'] = 'LU_updateUser_callback error: Sorry, your nonce did not verify.';
        }
        if( !current_user_can ('is_director') && !current_user_can ('is_sales_rep') && !current_user_can('is_sales_manager') )
        {
            $result['display_errors'] = 'failed';
            $result['success'] = false;
            $result['errors'] = 'LU_updateuser_callback Error: Sorry, you do not have permisison to view this page.';
        }
        else 
        {
            $response = LU_updateUser($portal_subdomain , $data);
                ...
                if ($user_id) 
                {
                    ...
                    if ($email != $old_email)
                    {
                        ...
                        else
                        {
                            ...
                            $result['display_errors'] = 'failed';
                            $result['success'] = false;
                            $result['errors'] = 'LU_updateUser_callback Error: Could not update WP user.';
                        }
                    }
...
/*
                ...
                else
                {
                    ...
                    $result['display_errors'] = 'failed';
                    $result['success'] = false;
                    $result['errors'] = 'LU_updateUser_callback Error: Could not create WP user.';
...
*/       
            ...
            else 
            {
                $result['display_errors'] = 'failed';
                $result['success'] = false;
                $result['errors'] = 'LU_updateUser_callback Error: There is an error updating user: ' . $response['message'];
            }
            ...
            if (isset($result['success']) && $result['success'])
            {
              ...
              if (isset($_REQUEST['portal_id']) && isset($_REQUEST['old_camp_name']) && isset($_REQUEST['camp_name']) && $_REQUEST['old_camp_name'] != $_REQUEST['camp_name'])
              {
                ...
                else
                {
                  ...
                  $result['display_errors'] = 'failed';
                  $result['success'] = false;
                  $result['errors'] = 'LU_updateUser_callback Error: There is an error updating users portal: ' . $response['message'];
                }
              }
            }
        }
    ...
    else
    {
        $result['display_errors'] = 'failed';
        $result['success'] = false;
        $result['errors'] = 'LU_updateUser_callback Error: Missing some parameters.';
    }
 ...
 */  
add_action('wp_ajax_LU_createUser', 'LU_createUser_callback'); //handles actions and triggered when the user is logged in

function LU_createUser_callback() {
        ...
        */
        else 
        {
            ...
            */
            ...
            {
                $response = LU_createUser($portal_subdomain, $data); // create the user in LU
...
*/                    
                    ...
                    if($course_name)
                    {
                        ...
                        $response = LU_enrollUserInCourse($email, $portal_subdomain, $data);    
                        ...
                        else
                        {
                            ...
                            $result['errors'] = "LU_CreateUser_callback Error: " . $response['message'];
                        }
                    }
                    else
                    {
                        $result['success'] = false;
                        $result['display_errors'] = true;
                        $result['errors'] = "LU_createUser_callback Error: Created user but could not enroll in course because couldn't find the course name.";
                    }   
                }
                // Expected error
                ...
                else 
                {
                    $result['success'] = false;
                    $result['display_errors'] = 'Failed';
                    $result['message'] = "LU_createUser_callback ERROR: could not create the staff account. Please contact the site administrator";
                }
            }
            else 
            {
 ...
 */  
add_action('wp_ajax_LU_deleteCourse', 'LU_deleteCourse_callback'); //handles actions and triggered when the user is logged in

function LU_deleteCourse_callback() {
    if( isset ( $_REQUEST['group_id'] ) && isset ( $_REQUEST['portal_subdomain'] ) && isset ( $_REQUEST['org_id'] ) )
    {
      ...
      if( ! wp_verify_nonce( $_POST['_wpnonce'] ,  'delete-course_' . $course_id ) ) 
      {
          $result['display_errors'] = 'failed';
          $result['success'] = false;
          $result['errors'] = 'LU_deleteCourse_callback error: Sorry, your nonce did not verify.';
      }
      if( !current_user_can ('is_director') && !current_user_can ('is_sales_rep') && !current_user_can('is_sales_manager') )
      {
          $result['display_errors'] = 'failed';
          $result['success'] = false;
          $result['errors'] = 'LU_deleteCourse_callback Error: Sorry, you do not have permisison to view this page.';
      }
      else 
      {
        $response = LU_deleteCourse($course_id, $portal_subdomain, $data);
...
add_action('wp_ajax_LU_getModules', 'LU_getModules_callback'); // Executes Courses_Modules functions actions only for log in users

function LU_getModules_callback() {
    if( isset ( $_REQUEST['course_id'] ) && isset ( $_REQUEST['portal_subdomain'] ) && isset ( $_REQUEST['org_id'] ) && isset ( $_REQUEST['subscription_id'] ) && isset( $_REQUEST['course_status'] )  )
    {
        ...
        else
        {
            ...
            $modules = LU_getModules( $course_id, $portal_subdomain, $info_data );
 ...
 *******************************************************************************************************/
add_action('wp_ajax_LU_getCourseForm', 'LU_getCourseForm_callback'); 
function LU_getCourseForm_callback ( ) {
    if(isset($_REQUEST['org_id']) && isset($_REQUEST['form_name']) )
    {
        ...
        ?>
            ...
            <div class="middle">
                <form id= "create_staff_group" frm_name="create_staff_group" frm_action="LU_createCourse" rel="submit_form" hasError=0> 
        ...
        ?>
            ...
            <div class="middle">
                <form id= "edit_staff_group" frm_name="edit_staff_group" frm_action="LU_updateCourse" rel="submit_form" hasError=0> 
                    <table padding=0 class="form"> 
                        <tr> 
                          <td class="label"> 
                            <label for="field_name">Name:</label> 
        ...
        else if($form_name == "LU_delete_course")
        {
            $course_id = filter_var($_REQUEST['course_id'],FILTER_SANITIZE_NUMBER_INT);
        ...
        ?>
            ...
            <div class="middle">
                <form id= "delete_staff_group" frm_name="delete_staff_group" frm_action="LU_deleteCourse" rel="submit_form" hasError=0> 
                    <table padding=0 class="form"> 
                        <tr >
                            <TD></TD>
                            <td class="value">
      ...
      -->
        ...
        else if($form_name == "add_video_group")
        {
            ...
            $course_modules = LU_getModules($course_id, $portal_subdomain, $data); // all the modules in the specified course
if (empty($course_modules))
{
  $course_modules = array();
            ...
            $modules_in_portal = LU_getModules(0,$portal_subdomain,$data); // all the modules in this portal
            $user_modules_titles = array_column($modules_in_portal, 'title'); // only the titles of the modules from the user library (course).
            $master_course = LU_getCourseByName(LE_LIBRARY_TITLE, $portal_subdomain, $data); // Get the master course. Cloned LE
            $master_course_id = $master_course['id']; // Master library ID
            $master_modules = LU_getModules($master_course_id, $portal_subdomain, $data); // Get all the modules from the master library (course).
...
-->                              
        ...
        ?>
            ...
            <div class="middle">
                <form id= "edit_staff_account" frm_name="edit_staff_account" frm_action="LU_updateUser" rel="submit_form" hasError=0> 
        ...
        else if($form_name == "create_staff_account")
        {
            $portal_subdomain = filter_var($_REQUEST['portal_subdomain'],FILTER_SANITIZE_STRING);            
            $data = array( "org_id" => $org_id );
            $courses_in_portal = LU_getCourses($portal_subdomain, '0', $data); // get all the published courses in the portal
        ...
        ?>
            ...
            <div class="middle">
              <div class="fixed_fb_width">
                <form id="create_staff_account" frm_name="create_staff_account" frm_action="LU_createUser" rel="submit_form" hasError=0> 
                  <br />
                  <br />
                  <span class="asterisk">*</span> Required fields            
                  <br />
        ...
        ?>
            ...
            <div class="middle" style ="font-size:12px;width:600px;margin:10px;padding:0px;clear:both;">
                <form frm_name="send_message" id="send_message" frm_action="LU_sendMail" rel="submit_form" hasError=0>
                    <table>
                      <tr>
        ...
        {        
            ...
            if($response['status'] == 1)
            {
                ...
                $enrollments = LU_getEnrollment( $course_id, $org_subdomain, $data ); // The lists of enrollment status including the info of the users registered in the course.
...
-->
                ...
                {
                     echo "LU_getCourseForm_callback error: add_staff_to_group form: " . $enrollments['message'];
                }
            }
        ...
        ?>
            ...
            <div class="middle publish_modules">
                ...
                <p>
                  <ol>
<?php
                    $modules = LU_getModules($course_id, $portal_subdomain, compact("org_id"));
                    foreach ($modules as $module)
                    {
...
jane@email.com
        ...
        ?>
            ...
            <div class="middle">
              <form id= "delete_staff_account" frm_name="delete_staff_account" frm_action="LU_deleteStaffAccount" rel="submit_form" hasError=0> 
                <table padding=0 class="form"> 
                  <tr> 
                    <td class="value"> 
        ...
        else if($form_name == "edit_questionnaire_conditions")
        {
            ...
            //List of all courses
            $courses = LU_getCourses(DEFAULT_SUBDOMAIN, 1);
                    ...
                    <form id= "edit_questionnaire_conditions" frm_name="edit_questionnaire_conditions" rel="submit_form" hasError=0>
                        <table padding=0 class="condition-form" data-course-name-id="1">
                            ...
                            <?php
                                ...
                                $new_staff_modules = LU_getModules($course_ids['New Staff'], DEFAULT_SUBDOMAIN);
                    ...
                    <form id= "edit_questionnaire_conditions" frm_name="edit_questionnaire_conditions" frm_action="editQuestionnaireConditions" rel="submit_form" hasError=0>
                        <table padding=0 class="condition-form" data-course-name-id="2">
                            ...
                            <?php
                                ...
                                $returning_staff_modules = LU_getModules($course_ids['Returning Staff'], DEFAULT_SUBDOMAIN);
                    ...
                    <form id= "edit_questionnaire_conditions" frm_name="edit_questionnaire_conditions" frm_action="editQuestionnaireConditions" rel="submit_form" hasError=0>
                        <table padding=0 class="condition-form" data-course-name-id="3">
                            ...
                            <?php
                                ...
                                $program_staff_modules = LU_getModules($course_ids['Program Staff'], DEFAULT_SUBDOMAIN);
                    ...
                    <form id= "edit_questionnaire_conditions" frm_name="edit_questionnaire_conditions" frm_action="editQuestionnaireConditions" rel="submit_form" hasError=0>
                        <table padding=0 class="condition-form" data-course-name-id="4">
                            ...
                            <?php
                                ...
                                $supervisory_staff_modules = LU_getModules($course_ids['Supervisory Staff'], DEFAULT_SUBDOMAIN);
 ...
 *******************************************************************************************************/
add_action('wp_ajax_LU_createCourse', 'LU_createCourse_callback'); 
function LU_createCourse_callback ( ) {
    if( isset ( $_REQUEST['org_id'] ) && isset ( $_REQUEST['name'] ) && isset ( $_REQUEST['user_id'] ) && isset ( $_REQUEST['subscription_id'] ) )
    {
        // This form is generated in LU_getCourseForm function from this file.
        $org_id = filter_var($_REQUEST['org_id'],FILTER_SANITIZE_NUMBER_INT);
        ...
        if( ! wp_verify_nonce( $_POST['_wpnonce'] ,  'create-course_' . $org_id ) ) 
        {
            $result['display_errors'] = 'failed';
            $result['success'] = false;
            $result['errors'] = 'LU_createCourse_callback error: Sorry, your nonce did not verify.';
        }
        else if(empty($_REQUEST['name']))
        {
            $result['display_errors'] = 'failed';
            $result['success'] = false;
            $result['errors'] = 'LU_createCourse_callback error: Please Enter the <b>Name</b> of the course.';
        }
        if( !current_user_can ('is_director') && !current_user_can ('is_sales_rep') )
        {
            $result['display_errors'] = 'success';
            $result['errors'] = 'LU_createCourse_callback error: Sorry, you do not have permisison to view this page. ';
        }
        else 
        {
            ...
            // Add the course
            $response = LU_createCourse($course_name, $org_subdomain, $data );
            if($response['status'] == 0)
            {
 ...
 *******************************************************************************************************/
add_action('wp_ajax_LU_updateCourse', 'LU_updateCourse_callback'); 
function LU_updateCourse_callback ( ) {
    if( isset ( $_REQUEST['org_id'] ) && isset ( $_REQUEST['name'] ) )
    {
        // This form is generated in LU_getCourseForm function with $form_name = edit_course_group from this file.
        $org_id = filter_var($_REQUEST['org_id'],FILTER_SANITIZE_NUMBER_INT);
        ...
        else 
        {
            ...
            // Edit the course
            $response = LU_updateCourse($course_id, $portal_subdomain, $data);
            if ($response['status'] == 0)
            {
                // return an error message
                $result['display_errors'] = true;
 ...
 *******************************************************************************************************/
add_action('wp_ajax_LU_sendMail', 'LU_sendMail_callback'); 
function LU_sendMail_callback() {
    $org_id = filter_var($_REQUEST['org_id'],FILTER_SANITIZE_NUMBER_INT);
    $target = filter_var($_REQUEST['target'],FILTER_SANITIZE_STRING);
    ...
    if( !current_user_can ('is_director') && !current_user_can ('is_sales_rep') && !current_user_can('is_sales_manager') )
    {
        $result['display_errors'] = 'Failed';
        $result['success'] = false;
        $result['errors'] = 'wp_ajax_LU_sendMail error: Sorry, you do not have permisison to view this page. ';
        return $result;
    }
...
//        $recepients = array_merge($recepients, array($recepient['email'] => $recepient));
        ...
        $response = LU_sendMail( $target, $recepients, $data );
        ...
        if($response['status'] == "0")
        {
            ...
            $result['display_errors'] = true;
            $result['success'] = false;
            $result['errors'] = "Error in LU_sendMail_callback: " . $response['message'];
        }
        else if($response['status'] == "1")
        {
        ...
        else
        {
            $result['display_errors'] = true;
            $result['success'] = false;
            $result['errors'] = "ERROR in LU_sendMail_callback: Could not email the user";
        }
    echo json_encode($result);
    }
    wp_die();
 ...
 *******************************************************************************************************/
add_action('wp_ajax_LU_getUsersInCourse', 'LU_getUsersInCourse_callback'); 
function LU_getUsersInCourse_callback() {
    if( isset ( $_REQUEST['course_id'] ) && isset ( $_REQUEST['org_id'] ) && isset ( $_REQUEST['portal_subdomain'] ) )
    {
        ...
        else
        {
            ...
            $users = LU_getUsersInCourse($course_id, $portal_subdomain, $info_data);
...
function changeCourseStatus_callback () {
    ...
    {
        // This form is generated in LU_getCourseForm function with $form_name = change_course_status_form from this file.
        $org_id = filter_var($_REQUEST['org_id'],FILTER_SANITIZE_NUMBER_INT);
        $status  = filter_var($_REQUEST['status'],FILTER_SANITIZE_STRING);
        $course_id = filter_var($_REQUEST['group_id'],FILTER_SANITIZE_NUMBER_INT);
        $portal_subdomain = filter_var($_REQUEST['portal_subdomain'],FILTER_SANITIZE_STRING);
 ...
 *******************************************************************************************************/
add_action('wp_ajax_LU_toggleItemInAssignment', 'LU_toggleItemInAssignment_callback'); 
function LU_toggleItemInAssignment_callback() {
    ...
    if( isset ( $_REQUEST['group_id'] ) && isset ( $_REQUEST['org_id'] ) && isset ( $_REQUEST['portal_subdomain'] ) && isset ( $_REQUEST['item'] ) && isset ( $_REQUEST['item_id'] ) )
    {
        ...
        // get modules of the course
        //LU_deleteModule($course_id, $portal_subdomain, $info_data);
        //$result['message'] = LU_AddModule($course_id, $portal_subdomain, $info_data);
        $response = LU_toggleItemInAssignment($course_id, $portal_subdomain, $info_data);
 ...
 *  @return json encoded list of enrollments
 */
function LU_getEnrollmentsByUserId($user_id = 0, $portal_subdomain = DEFAULT_SUBDOMAIN, $data = array()){
    extract($data);
    ...
    */
    ...
    {
        return array('status' => 0, 'message' => "ERROR in LU_getEnrollmentsByUserId: Invalid user id.");
    }
    ...
    if(isset($response['message'])) 
    {
        return array('status' => 0, 'message' => "LUERROR in LU_getEnrollmentsByUserId:" . $response['message']);
    }    
    else if (isset($response['enrollments'][0]['id'])) 
    {
        return array('status' => 1, 'enrollments' => $response['enrollments']);
 ...
 * @param string $portal_subdomain - The subdomain name of the portal
 **/
function LU_deleteEnrolledUser($enrollment_id = 0, $portal_subdomain = DEFAULT_SUBDOMAIN, $data) {
    extract($data);
    /*
    * Variables required in $data
    * org_id - the organization ID
    */
    ...
    if($enrollment_id == 0)
        return array('status' => 0, 'message' => "ERROR in LU_deleteEnrolledUser: invalid enrollment id");
    ...
    if(isset($response['message'])) 
    {
        return array('status' => 0, 'message' => "Error in LU_deleteEnrolledUser: " . $response['message']);
    }    
    else 
    {
        return array('status' => 1);
 ...
 *******************************************************************************************************/
add_action('wp_ajax_LU_enrollUserInCourse', 'LU_enrollUserInCourse_callback'); 
function LU_enrollUserInCourse_callback () {

    // This form is generated in LU_getCourseForm function with $form_name = add_staff_to_group from this file.
    $org_id = filter_var($_REQUEST['org_id'],FILTER_SANITIZE_NUMBER_INT); // The organization ID
    ...
    if(wp_verify_nonce( $_REQUEST['nonce'], 'process-userEmail_' . $email ) )
    {
        $result['display_errors'] = 'Failed';
        $result['success'] = false;
        $result['errors'] = 'LU_enrollUserInCourse_callback error: Sorry, your nonce did not verify.';
    }
    else if( !current_user_can ('is_director') && !current_user_can ('is_sales_rep') )
    {
        $result['display_errors'] = 'Failed';
        $result['success'] = false;
        $result['errors'] = 'LU_enrollUserInCourse_callback error: Sorry, you do not have permisison to view this page.';
    }
    else 
    {
        $data = compact( "org_id", "course_name");
        ...
        if ($response['status'] == 0)
        {
            ...
            $result['display_errors'] = true;
            $result['success'] = false;
            $result['errors'] = "LU_enrollUserInCourse_callback error: " . $response['message'];
        ...
        else
        {
            ...
            $result['display_errors'] = true;
            $result['success'] = false;
            $result['errors'] = "LU_enrollUserInCourse_callback : Could not enroll user in course";
        }
    }
    echo json_encode($result);
    wp_die();
...
function getVideosInGroup_callback() {
    if( isset ( $_REQUEST['group_id'] ) && isset ( $_REQUEST['org_id'] ) && isset ( $_REQUEST['portal_subdomain'] ) )
    {
        ...
        {
            $modules = LU_getModules( $course_id, $portal_subdomain, $info_data ); // All the modules in the course.
 ...
 *******************************************************************************************************/
add_action('wp_ajax_LU_deleteEnrolledUser', 'LU_deleteEnrolledUser_callback'); 
function LU_deleteEnrolledUser_callback () {
    ...
    if(wp_verify_nonce( $_REQUEST['nonce'], 'process-userEmail_' . $email ) )
    {
        $result['display_errors'] = 'Failed';
        $result['success'] = false;
        $result['errors'] = 'LU_deleteEnrolledUser_callback Error: Sorry, your nonce did not verify.';
    }
    else if( !current_user_can ('is_director') && !current_user_can ('is_sales_rep') )
    {
        $result['display_errors'] = 'Failed';
        $result['success'] = false;
        $result['errors'] = 'LU_deleteEnrolledUser_callback Error: Sorry, you do not have permisison to view this page.';
    }
    else 
    {
      $data = compact("org_id");

      $response = LU_deleteEnrolledUser($enrollment_id, $portal_subdomain, $data);
      if ($response['status'] == 0)
      {
          // return an error message
          $result['display_errors'] = true;
      ...
      else
      {
          ...
          $result['display_errors'] = true;
          $result['success'] = false;
          $result['errors'] = "LU_deleteEnrolledUser_callback ERROR: Could not delete the user from the course.";
      }
    }
    echo json_encode($result);
    wp_die();
 ...
 *******************************************************************************************************/
add_action('wp_ajax_LU_updateDueDate', 'LU_updateDueDate_callback'); 
function LU_updateDueDate_callback() {
    if( isset ( $_REQUEST['action'] ) && isset ( $_REQUEST['course_id'] ) && isset ( $_REQUEST['task'] ) && isset ( $_REQUEST['portal_subdomain'] ) && isset ( $_REQUEST['org_id'] ) )
    {
        if($_REQUEST['task'] == "remove" || $_REQUEST['task'] == "add")
        {
            ...
            if( !current_user_can ('is_director') && !current_user_can ('is_sales_rep') )
            {
                $result['display_errors'] = 'Failed';
                $result['success'] = false;
                $result['errors'] = 'LU_updateDueDate_callback Error: Sorry, you do not have permisison to view this page.';
            }
            else
            {
                ...
                if ($response['status'] == 0)
                {
                    $result['display_errors'] = true;
                    $result['success'] = false;
                    $result['errors'] = "LU_updateDueDate_callback ERROR: ". $response['message'];
                ...
                else
                {
                    ...
                    $result['display_errors'] = true;
                    $result['success'] = false;
                    $result['errors'] = "LU_updateDueDate_callback ERROR: Could not update the course.";
                }
            }
        }
        else
        {
            $result['display_errors'] = true;
            $result['success'] = false;
            $result['errors'] = "LU_updateDueDate_callback ERROR: invalid task.";
        }
    }
    else
    {
        $result['display_errors'] = true;
        $result['success'] = false;
        $result['errors'] = "LU_updateDueDate_callback ERROR: Missing parameters";
    }
    echo json_encode($result);
    wp_die();
}
 ...
 *
 * TEST AJAX CALLS
 * URL: http://eot.dev/wp-admin/admin-ajax.php?action=LU_TESTAJAX&portal_subdomain=winbaum&course_name=Expert Online Training Tutorial&course_id=66810&org_id=199
 *
 **************************************************************************/
add_action('wp_ajax_LU_TESTAJAX', 'LU_TESTAJAX_callback'); 
function LU_TESTAJAX_callback ( ) {
...
/*
  ...
  $course_modules_titles = array_column($course_modules, 'title'); // only the titles of the modules in the specified course
  $modules_in_portal = LU_getModules('0',$portal_subdomain,$data); // all the modules in this portal
  $master_modules = LU_getModules(lrn_upon_LE_Course_ID, DEFAULT_SUBDOMAIN, array()); // Get all the modules from the master library (course).
  $master_modules_titles = array_column($master_modules, 'title'); // only the titles of the modules from the master library (course).
  $course_data = getCourse($portal_subdomain, $course_id, $data); // all the settings for the specified course
 ...
 */  
function LU_enrollUserInCourses($portal_subdomain = DEFAULT_SUBDOMAIN, $courses = array(), $org_id = 0, $email = '')
{
  // make sure we have the data we need.
  if ($email == '' || !$org_id)
  {
    return array('status' => 0, 'message' => 'Error: Invalid arguments supplied to LU_enrollUserInCourses');
  }
  ...
  foreach ($courses as $course_name)
  {
    ...
    {
      $result['status'] = 0;
      $result['message'] = "LU_enrollUserInCourses Error: " . $response['message'];
      return $result;
    }
  }
  return array('status' => 1);
    ...
    else
    {
      ...
      else if (isset($group['status']) && $group['status'])
      {
        ...
        if ( email_exists($email) == false )
        {
          ...
          $response = LU_createUser($org_subdomain, $data); // create the user in LU

          if(isset($response['status']) && !$response['status'])
          {
            // got an error from LU_createUser
            $result['success'] = false;
            $result['display_errors'] = true;
...
function getStaffInCourse_callback () 
{
  ...
  if( isset($_REQUEST['ids']) )
  {
    ...
    {
      $enrollments = LU_getEnrollment($course_id, $org_subdomain, $data); // All enrollments in the courses 
      if( $enrollments )
      {
        foreach ($enrollments as $enrollment) 
        {
