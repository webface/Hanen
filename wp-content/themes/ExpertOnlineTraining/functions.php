...
function eot_enqueue_scripts() 
{
	...
	wp_enqueue_style('videojs-style', 'https://vjs.zencdn.net/6.2.7/video-js.min.css', '', '6.2.7');
	wp_enqueue_style('help-button-style', get_template_directory_uri() . '/css/help_button.css', '', '1.0.0');
    wp_enqueue_style('datatable-buttons',get_template_directory_uri() . '/css/buttons.dataTables.min.css', '', '1.4.0');
	...
	wp_enqueue_script('videojs', 'https://vjs.zencdn.net/6.2.7/video.min.js', '', '6.2.7');
    ...
    wp_enqueue_script('datatables-js-buttons', get_template_directory_uri() . '/js/dataTables.buttons.1.4.0.min.js', '', '1.4.0');
...
function unsubscribe_wp_mail_filter( $args ) {
	...
	if($user_email)
	{
		...
		$message .= '<a rel="nofollow" target="_blank" href="'.get_site_url().'/unsubscribe/?email='.$user_email.'&sec='.$hash_key.'" title="Unsubscribe" style="color:#006AC3;text-decoration:underline;">Unsubscribe</a>';
		$new_wp_mail = array(
			'to'          => $args['to'],
