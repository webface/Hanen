...
if ( SHOW_SQL ) error_log("after_submission_register: get_results-> $sql");
            ...
            if ( $num_rows == 1 )
            {
	            ...
	            $course_id = isset($current[0]['course_id']) ? $current[0]['course_id'] : '';
				$user_email = isset($current[0]['user_email']) ? $current[0]['user_email'] : $email;
            }
            else if ( $num_rows > 1 )
            {
            	...
            	{
            		if ( $invitation['user_email'] == $email )
            		{
			            ...
			            $course_id = isset($invitation['course_id']) ? $invitation['course_id'] : '';
						$user_email = isset($invitation['user_email']) ? $invitation['user_email'] : $email;
            		}
            	}
            }
            else
...
if ( SHOW_SQL ) error_log("after_submission_register: get_row-> $sql");
            ...
            { // org has enough staff credits
                add_user_meta($user_id, 'org_id', $org_id);
                if( $course_id )
                {
		...
		{ 	/* now director has been created, go ahead and add a new organization with the post_author variable being the new director id */
			...
			if (is_wp_error($org_id)) { /* if error creating new organization, output to director on screen */
				...
				if(!current_user_can( "is_sales_rep" ) && !current_user_can( "is_sales_manager" ))
				{
					...
					if (is_wp_error ($user)) 
					{ // if error, notify director there was an error logging in
						$result['status'] = 0;
